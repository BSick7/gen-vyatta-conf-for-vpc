var fs = require('fs'),
    path = require('path'),
    VYATTA_TEMPLATE_FILE = "../templates/vyatta.config",
    SETUP_TEMPLATE_FILE = "../templates/setup.sh";
...
function genConfig(replacements) {
    return replaceAll(replacements, fs.readFileSync(VYATTA_TEMPLATE_FILE));
}

function genSetupScript(replacements) {
    return replaceAll(replacements, fs.readFileSync(SETUP_TEMPLATE_FILE));
}
